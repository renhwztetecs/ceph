...
def test2(cluster):
    uid = "tester2"
    # XXX For some reason, a line with spaces causes a blow-up.
    #     Works for Yehuda though! Why?
...
def test4(cluster):
    ...
    #
    # XXX
    # This 100% works with standalone vstart:
    #   ./radosgw-admin key create --subuser='prodtx$prodt:prod1' --key-type=swift --secret=testpass
    swift_secret = "test3pass"
    # But this blows up with "user info was not populated":
    #cmd = t.build_cmd(
    #    '--subuser', "'%s$%s:%s'" % (tid, uid, subid),
    #    '--key-type', 'swift',
    #    '--secret', swift_secret,
    #    "key create")
    # Probably something related to shell expansion of the dollar.
    # Workaround -- note the unqualified subuser:
    cmd = t.build_cmd(
        "--tenant=%s" % tid,
        "--uid=%s" % uid,
        "--subuser='%s:%s'" % (uid, subid),
        '--key-type=swift',
        '--secret=%s' % swift_secret,
        "key create"
        )
    out, ret = cluster.rgw_admin(cmd, check_retcode=False)
    if ret != 0:
